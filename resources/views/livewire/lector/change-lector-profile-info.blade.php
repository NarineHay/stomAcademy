        ...
        <div class="mt-3 bg-white br-12 p-4" wire:ignore>
            ...
            <div class="collapse" id="profile-1">
                <div>
                    ...
                    <div class="mt-4">
                        <button wire:click=""
                            class="btn btn-primary py-2 px-4 br-12">{{ __('profile.profile.save') }}</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="mt-3 bg-white br-12 p-4" wire:ignore>
            ...
            <div class="collapse" id="profile-4">
                <div>
                    ...
                    <div class="mt-4">
                        <button wire:click="saveBiography"
                            class="btn btn-primary py-2 px-4 br-12">{{ __('profile.profile.save') }}</button>
                    </div>
                </div>
            </div>
                        ...
                        @foreach ($directions as $direction)
                            <div class="form-check">
                                <input type="checkbox" wire:model="userDirections" value="{{ $direction->id }}"
                                    @if ($user->directions->where('direction_id', $direction->id)->count()) checked @endif class="mr-1 form-check-input">
                                <label class="form-check-label">{{ $direction->title }}</label>
                            </div>
                        @endforeach
                    </div>
                    <div class="mt-3">
                        <button wire:click="directions"
                            class="btn btn-primary py-2 px-4 br-12">{{ __('profile.profile.save') }}</button>
                    </div>
                </div>
            </div>
