    ...
    <div class="col-lg-10 col-12 mb-4 mb-lg-6">
        ...
        <div class="d-flex justify-content-between flex-column flex-lg-row mt-4 mb-4 align-items-lg-center">
            ...
            <div class="col-12 d-flex d-lg-none justify-content-between mt-2 filter_buttons_mobile mb-2">
                ...
                <button class="fs-12 f-600 py-2 w-50 bg-transparent" type="button" data-bs-toggle="collapse"
                        ...
                        href="filter.html" class="text-black">{{ __("courses.filters.filter") }}</a></button>

                <button class="fs-12 f-600 py-2 w-50 bg-transparent text-black dropdown-toggle" type="button"
                        data-bs-toggle="dropdown" aria-expanded="false">{{ __("courses.sort.title") }}
                        @if($sortBy) @switch($sortBy) @case("default")  @break
                        @case("price_asc") {{ __("courses.sort.price_asc") }} @break
                        @case("price_desc") {{ __("courses.sort.price_desc") }} @break
                        @case("title") {{ __("courses.sort.name") }} @break
                        @case("popularity") {{ __("courses.sort.pop") }} @break @endswitch @endif
                </button>
                <ul class="dropdown-menu p-2">
                    <div class="d-flex align-items-center">

                    <input wire:model="sortBy" value="popularity" type="radio" id="inp-popularity-1" name="sort"
                           class=" cursor">
                    <label for="inp-popularity-1" class="f-500 fs-14 ms-2 cursor">{{ __("courses.sort.pop") }}</label><br>
                    </div>
                    <div class="d-flex">

                    <input wire:model="sortBy" value="price_desc" type="radio" id="inp-price_desc-1" name="sort"
                        class="cursor">
                    <label for="inp-price_desc-1" class="f-500 fs-14 ms-2 cursor">{{ __("courses.sort.price_desc") }}</label><br>
                    </div>
                    <div class="d-flex">

                    <input wire:model="sortBy" value="price_asc" type="radio" id="inp-price_asc-1" name="sort"
                           class=" cursor">
                    <label for="inp-price_asc-1" class="f-500 fs-14 ms-2 cursor">{{ __("courses.sort.price_asc") }}</label><br>
                    </div>
                    <div class="d-flex">

                    <input wire:model="sortBy" value="title" type="radio" id="inp-title-1" name="sort"
                           class=" cursor">
                    <label for="inp-title-1" class="f-500 fs-14 ms-2 cursor">{{ __("courses.sort.name") }}</label><br>
                    </div>
                </ul>
            </div>
            ...
            <div class="d-flex align-items-center d-none d-lg-block">
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu p-3 border-0" aria-labelledby="dropdownMenuButton1" style="width: 200px">
                        <div class="d-flex">
                            <input wire:model="sortBy" value="popularity" type="radio" id="inp-popularity-2" name="sort"
                                class=" cursor">
                            <label for="inp-popularity-2" class="f-500 fs-14 ms-2 cursor">{{ __("courses.sort.pop") }}</label><br>
                        </div>
                        <div class="d-flex">

                        <input wire:model="sortBy" value="price_desc" type="radio" id="inp-price_desc-2" name="sort"
                        class=" cursor">
                        <label for="inp-price_desc-2" class="f-500 fs-14 ms-2 cursor">{{ __("courses.sort.price_desc") }}</label><br>
                        </div>
                        <div class="d-flex">

                        <input wire:model="sortBy" value="price_asc" type="radio" id="inp-price_asc-2" name="sort"
                           class="cursor">
                        <label for="inp-price_asc-2" class="f-500 fs-14 ms-2 cursor">{{ __("courses.sort.price_asc") }}</label><br>
                        </div>
                        <div class="d-flex">

                        <input wire:model="sortBy" value="title" type="radio" id="inp-title-2" name="sort"
                               class=" cursor">
                        <label for="inp-title-2" class="f-500 fs-14 ms-2 cursor">{{ __("courses.sort.name") }}</label><br>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="row mt-4">
            ...
            @foreach($courses as $course)
                ...
                <a href="@if($type == "webinars") {{ route("webinar.show",$course->info->slug) }} @else {{ route("course.show",$course->info->slug) }} @endif" style="color: inherit"
