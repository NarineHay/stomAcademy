    ...
    <div class="col-lg-10 col-12 mb-4 mb-lg-6">
        <div class="d-flex mt-6 py-lg-2">
            <a href="{{route('home')}}"><span
                    class="fs-12 f-500 text-secondary cursor">{{ __("header.menu.home") }}</span></a>
                ...
                <div class="dropdown">
                    ...
                    <div class="dropdown-menu p-3 border-0" aria-labelledby="dropdownMenuButton1">
                        ...
                        <label for="vehicle111"
                               class="f-500 fs-14 ms-2 cursor">{{ __("courses.sort.price") }}</label><br>
                        <input type="radio" id="vehicle1112" name="vehicle" class="mt-2 cursor">
                        <label for="vehicle1112"
                               class="f-500 fs-14 ms-2 cursor">{{ __("courses.sort.name") }}</label><br>
                        <input type="radio" id="vehicle3333" name="vehicle" class="mt-2 cursor">
                        <label for="vehicle3333"
                               class="f-500 fs-14 ms-2 cursor">{{ __("courses.sort.pop") }}</label><br>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <div
                                ...
                                <div class="mt-3 mt-xl-0 d-flex flex-row flex-xl-column">
                                    @if($course->sale)
                                        ...
                                        <span class="f-700 text-secondary fs-16 text-nowrap del">{{$course->price->html()}}</span>
                                    @else
        ...
        <button wire:click="loadNext"
                class="w-100 fs-14 f-500 mt-3 mt-lg-6 py-3 br-12 show_more_btn bg-transparent text-black">{{ __("index.show_more") }}</button>
            ...
            <div>
                <div class="mt-4 ms-3 pt-5">
                    <label class="f-600 fs-16 d-flex justify-content-between align-items-center fg-label cursor"
                           ...
                           data-bs-target="#fg-1"><span>{{ __("courses.filters.directions") }}</span><i
                ...
                <div class="mt-4 ms-3">
                    <label class="f-600 fs-16 d-flex justify-content-between align-items-center fg-label cursor"
                           data-bs-toggle="collapse"
                           data-bs-target="#fg-3"><span>{{ __("courses.filters.price") }}</span><i
                            class="fal fa-angle-right"></i></label>
                ...
                <div class="mt-4 ms-3">
                    <label class="f-600 fs-16 d-flex justify-content-between align-items-center fg-label cursor"
                           data-bs-toggle="collapse"
                           data-bs-target="#fg-2"><span>{{ __("courses.filters.lector") }}</span><i
                            class="fal fa-angle-right"></i></label>
